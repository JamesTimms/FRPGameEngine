...
vec2 texCoordForFace(vec3 hit, const box b, int fIndex) {
  if (fIndex == CUBE_FACE_POS_X) {
    vec2 misc.lwjgl.res = (hit.zy - b.min.zy) / (b.max.zy - b.min.zy);
    return vec2(1.0 - misc.lwjgl.res.x, 1.0 - misc.lwjgl.res.y);
  } else if (fIndex == CUBE_FACE_NEG_X) {
    vec2 misc.lwjgl.res = (hit.zy - b.min.zy) / (b.max.zy - b.min.zy);
    return vec2(misc.lwjgl.res.x, 1.0 - misc.lwjgl.res.y);
    
  } else if (fIndex == CUBE_FACE_POS_Y) {
    vec2 misc.lwjgl.res = (hit.xz - b.min.xz) / (b.max.xz - b.min.xz);
    return misc.lwjgl.res;
  } else if (fIndex == CUBE_FACE_NEG_Y) {
    vec2 misc.lwjgl.res = (hit.xz - b.min.xz) / (b.max.xz - b.min.xz);
    return misc.lwjgl.res;
    
  } else if (fIndex == CUBE_FACE_POS_Z) {
    vec2 misc.lwjgl.res = (hit.xy - b.min.xy) / (b.max.xy - b.min.xy);
    return vec2(misc.lwjgl.res.x, 1.0 - misc.lwjgl.res.y);
  } else {
    vec2 misc.lwjgl.res = (hit.xy - b.min.xy) / (b.max.xy - b.min.xy);
    return vec2(1.0 - misc.lwjgl.res.x, 1.0 - misc.lwjgl.res.y);
